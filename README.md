...
## 7. 常见问题
- 模型下载问题: 很多用户的机器是购买或者租用的云主机，网络会有一定的限制，导致whisper/chatglm模型不能自动下载或者下载很慢，可以尝试手动下载模型文件并修改代码中的模型目录
  - 首先，进入HuggingFace下载页面，点击下载按钮把目录内所有文件下载到本机. fast_whisper下载地址(点击"Files and Versions")：`https://huggingface.co/guillaumekln/faster-whisper-medium/tree/main`; chatglm下载地址(点击"Files and Versions")：`https://huggingface.co/THUDM/chatglm-6b-int4`
  - 然后，把下载好的文件scp到云主机上, 本机执行命令`scp -P port_id user_name@ip:dir_name`（都替换成你机器的名字和地址）
  - 最后，修改`app.py`中模型路径地址为手动下载模型的绝对路径地址dir_name, whisper模型路径代码行是`whisper_model="medium"`, chatglm模型路径代码行是`model_name = "THUDM/chatglm-6b-int8"`
